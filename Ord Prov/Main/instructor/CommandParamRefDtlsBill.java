 ...
 * 23/09/2013					Ravikumar G				defect#162: Exception in populating cmd trans details for param: ExternalIDType
 ******************************************************************************/

public class CommandParamRefDtlsBill {
	...
	public List<HandlerVariables> getParamValues(String cmdRefId, List<HandlerVariables> paramList, XPath xpath, Document doc, JdbcDatabaseDAO jdbcDatabaseDAO, String orderXml) throws SPLExceptionHandler {
		...
		try{
			...
			for(Object list:cmdParamRefList){
				...
				taLogger.log(var.getSrcTransId(), var.getTrans_Id(),var.getRow_ID(), ApplicationConstants.LOG_DEBUG, "[getParamValues] splFunction in paramVariableList: "+paramVariableList.size(), loggercategory);
				...
				// Ravi: defect#162: no need to call spl function if list is empty
				if(functionVariable != null && paramVariableList.size()>0){
					...
					paramVariableList = getNewParamValuesforSplFunction(paramVariableList,functionVariable,orderXml);
				...
				{
					for (int i = 0; i < paramVariableList.size(); i ++){
						cmdtransDtlsMap.put(paramName, paramVariableList.get(i));
			...
			taLogger.log(var.getSrcTransId(), var.getTrans_Id(),var.getRow_ID(), ApplicationConstants.LOG_DEBUG, "[getParamValues] inside catch block " +e.getMessage(), loggercategory, e);
	...
	public List<HandlerVariables> getNewParamValuesforSplFunction(List<HandlerVariables> inputParamList, String functionVariable, String orderXml) throws Exception {
